...
namespace WebApi.DataBase.Oracle
{
	...
	/// </summary>
	/// <threadsafety ="true"/>
	public class CDataBase : IDataBaseService
	{
		...
		/// </summary>
		private DbProviderFactory modProvider;

		private readonly CTransaction mvGlobalTransaction;
		...
		/// </summary>
		//private  List<DbConnection> modConnectionsForTransactions = new List<DbConnection>(modMaxCountOfSimultaneousTransactions);
		...
		/// </summary>
		//private  List<object> modConnectionsForTransactionsMutexes = new List<object>(modMaxCountOfSimultaneousTransactions);

		/// <summary>
		/// Command builder, use it for deriving command parameters and build SQL queries
		/// </summary>
		private DbCommandBuilder modDBCommandBuilder;

		/// <summary>
		/// Connection to DBMS
		/// </summary>
		private DbConnection modDBConnection;

		/// <summary>
		/// Transaction TODO: Need to do changes in logic of transactions
		/// </summary>
		private DbTransaction modTransaction;

		/// <summary>
		/// Cache of stored procedures/function parameters.
		/// 
		/// </summary>
		private Hashtable modParametersCache;

		/// <summary>
		/// Last Oracle error message
		/// </summary>
		private string mvDBMessage;
		...
		/// </summary>
		private MethodInfo modDeriveParameters;

		/// <summary>
		/// Oracle scheme name
		/// </summary>
		private string mvDataSource = String.Empty;

		/// <summary>
		/// DBMS User Name
		/// </summary>
		private string mvUserID = "adaica";
		...
		/// User password for mvUserID user.
		/// </summary>
		//private  string mvPasswordCryptDefault = "Wfmc631p/HXqgqw30m/0Zw==";
		private string mvPasswordCryptDefault = "WYybngNlhXnuhqgz1mvwYw==";
		private string OracleDataAccessClient = "Oracle.DataAccess.Client";



		//Default Login
		private string mvDefaultSchemaUser = "ADAICA_CONNECT";
		private string mvDefaultSchemaPassword = "TIyMngVllHnfjtY7uGOQa6Ye+wqqKnqbT3NYCIPCXg8=";

		/// <summary>
		/// Whether we already said to user that we cant establish connection
		/// </summary>
		bool alreadySaidAboutNullConnection = false;

		private bool modIsStartConnection = true;

		//private  Dictionary<string, DbConnection> modTransactionConnections = null;
		private Dictionary<string, CDbConnectionProperties> mvTransactionConnections = null;
		private Dictionary<string, object> modTransactionConnectionsMutexes = null;

		private string mvCurrentRealUserId;

		private DateTime mvLastSuccessRequestTime;
		...
		void Alert(string mess, Exception exc)
		{
			...
			try
			{
				...
				catch (Exception exc)
				{
					...
					try
					{
						...
						//amb = Assembly.LoadFrom(@"C:\oraclecomponents\odp.net\bin\4\Oracle.DataAccess.dll");
						//amb = Assembly.LoadFrom(@"D:\oracle\client_2\bin\oracle.dataaccess.dll");
						//amb = Assembly.LoadFrom(@"D:\oracle\client_2\odp.net\bin\4\oracle.dataaccess.dll");
		...
		/// </summary>
		public CTransaction GlobalTransaction
		{
			[DebuggerStepThrough]
			get { return mvGlobalTransaction; }
		}

		public string CurrentDatabaseHostName
		{
			get
			{
		...
		private void ExtractCommandParametersString(CDBParameters commandParameters, out string paramsString, out string paramsParamString)
		{
		...
		private void CloseConnections(object sender, ExitEventArgs e)
		{
			if (mvIsTraceNeedOn)
		...
		private bool isConnectOk(DbConnection connection)
		{
		...
		/// </returns>
		private bool isConnectOk(DbConnection connection, bool tryToConnect)
		{
			if (connection == null)
		...
		/// </returns>
		private bool isConnectOk(bool tryToConnect)
		{
		...
		private void attachParameters(DbCommand command)
		{
			try
			{
		...
		private Thread callInThread(ref ExampleCallback callback, object[] args_p)
		{
		...
		void threadedCall(object args1)
		{
		...
		public string ConnectionString(bool isShowPassword)
		{
		...
		bool connect(DbConnection connection)
		{
			bool result = false;

			try
			{
				CLog.Log("GET connect IN");
		...
		private void OnConnectSuccessfully(DbConnection con)
		{
			//set real user context
			if (!String.IsNullOrEmpty(mvCurrentRealUserId))
			{
				CDBParameters parameters;
				string procedureName = modSQL.sqlRealUserContextSet(mvCurrentRealUserId, out parameters);
		...
		/// <param name="connection">The connection.</param>
		/// <returns></returns>
		bool TryConnect(DbConnection connection)
		{
			CLog.Log("GET TryConnect IN");
			bool res = false;
			if (init(connection))
			{
				try
				{
					CLog.Log("GET tryConnect with connection string" + connection.ConnectionString);
					IsSchemaPasswordNotCorrect = false;
					connection.Open();
					CLog.Log("GET tryConnect connectionOpened");
				...
				catch (Exception exc)
				{
					try
					{
						...
						else
						{
							...
							{
								CLog.Log("Time out connection." + exc.Message);
		...
		public Dictionary<string, CDbConnectionProperties> TransactionConnections
		{
		...
		/// }
		/// private  void TestTransaction( object objTransaction )
		/// {
		...
		private string GetConnectionIdForTransaction()
		{
		...
		/// </value>
		public bool IsSchemaPasswordNotCorrect
		{
			get;
			set;
		}
		...
		/// </value>
		public bool IsLocalSchemaPasswordNotCorrect
		{
			get;
			set;
		}

		public string DefaultSchemaUser
		{
			get { return mvDefaultSchemaUser; }
			set { mvDefaultSchemaUser = value; }
		}

		public string DefaultSchemaPassword
		{
		...
		public string PasswordCryptDefault
		{
		...
		public bool AlreadySaidAboutNullConnection
		{
		...
		/// <returns>Database session identificator</returns>
		public int Sid(DbConnection connection)
		{
			int sid = -1;
		...
		public bool init(DbConnection connection)
		{
			bool res = false;
			if ((connection == null) ||
		...
		/// <returns>Result of execution stored procedure/function</returns>
		public object getScalar(string commandText, DbConnection connection)
		{
		...
		public object getScalar(string commandText, CommandType commandType, CTransaction transaction)
		{
		...
		public bool ExecuteAll(CTransaction transaction, params CDBQuery[] queries)
		{
			for (int i = 0; i < queries.Length; i++)
			{
				var res = (int)getScalar(queries[i], transaction);
		...
		public bool ExecuteAll(CTransaction transaction, IEnumerable<CDBQuery> queries)
		{
			foreach (var query in queries)
			{
				var res = (int)getScalar(query, transaction);
		...
		public object getScalar(string commandText, CDBParameters parameters, CTransaction transaction)
		{
		...
		/// </returns>
		public object getScalar(string commandText, CommandType commandType, DbConnection connection)
		{
			return getScalar(commandText, commandType, (CDBParameters)null, connection, null);
		}

		public object getScalar(string commandText, CommandType commandType, DbConnection connection, CTransaction transaction)
		{
			return getScalar(commandText, commandType, (CDBParameters)null, connection, transaction);
		}

		public object getScalar(string commandText, CommandType commandType, DbConnection connection, CTransaction transaction, CCommandWrapper cmdWrapper)
		{
		...
		/// <returns></returns>
		public object getScalar(string commandText, CDBParameters commandParameters)
		{
		...
		/// </returns>
		public object getScalar(string commandText, CDBParameters commandParameters, DbConnection connection)
		{
		...
		/// <returns>Result of execution stored procedure/function</returns>
		public object getScalar(string commandText, CommandType commandType, CDBParameters commandParameters)
		{
		...
		/// <returns></returns>
		public object getScalar(string commandText, CommandType commandType, CDBParameters commandParameters, DbConnection connection, CTransaction transaction)
		{
		...
		/// Result of execution SQL query or stored procedure/function
		/// </returns>
		public object getScalar(string commandText, CommandType commandType, CDBParameters commandParameters, DbConnection connection, CTransaction transaction, CCommandWrapper cmdWrapper)
		{
			object result = null;
		...
		public DbDataReader getReader(string commandText, CCommandWrapper cmdWrapper)
		{
		...
		/// <returns></returns>
		public DbDataReader getReader(string commandText, DbConnection connection)
		{
			return getReader(commandText, connection, (CCommandWrapper)null);
		}
		public DbDataReader getReader(string commandText, DbConnection connection, CCommandWrapper cmdWrapper)
		{
		...
		/// or CommandType.Text</param>
		/// <returns>A result set of records</returns>
		public DbDataReader getReader(string commandText, CommandType commandType)
		{
			return getReader(commandText, commandType, (CCommandWrapper)null);
		}
		public DbDataReader getReader(string commandText, CommandType commandType, CCommandWrapper cmdWrapper)
		{
		...
		/// <param name="connection">Connection on which command should be run</param>
		/// <returns></returns>
		public DbDataReader getReader(string commandText, CommandType commandType, DbConnection connection)
		{
			return getReader(commandText, commandType, connection, (CCommandWrapper)null);
		}
		public DbDataReader getReader(string commandText, CommandType commandType, DbConnection connection, CCommandWrapper cmdWrapper)
		{
			return getReader(commandText, commandType, (CDBParameters)null, connection, cmdWrapper, (CTransaction)null);
		}

		public DbDataReader getReader(string commandText, CommandType commandType, DbConnection connection, CCommandWrapper cmdWrapper, CTransaction transaction)
		{
		...
		/// <returns>A result set of records</returns>
		public DbDataReader getReader(string commandText, CDBParameters commandParameters)
		{
			return getReader(commandText, commandParameters, (CCommandWrapper)null);
		}

		public DbDataReader getReader(CDBQuery dbQuery)
		{
			return getReader(dbQuery.ProcedureName, dbQuery.Parameters);
		}

		public DbDataReader getReader(CDBQuery dbQuery, CTransaction transaction)
		{
			return getReader(dbQuery.ProcedureName, dbQuery.Parameters, null, null, transaction);
		}

		public DbDataReader getReader(string commandText, CDBParameters commandParameters, CTransaction transaction)
		{
			return getReader(commandText, commandParameters, null, null, transaction);
		}

		//+++++++++++++++++++++++++++++++++++++++
		public DbDataReader getReader(string commandText, CDBParameters commandParameters, CCommandWrapper cmdWrapper)
		{
		...
		/// <param name="connection">Connection on which command should be run</param>
		/// <returns></returns>
		public DbDataReader getReader(string commandText, CDBParameters commandParameters, DbConnection connection)
		{
			return getReader(commandText, commandParameters, connection, (CCommandWrapper)null);
		}
		public DbDataReader getReader(string commandText, CDBParameters commandParameters, DbConnection connection, CCommandWrapper cmdWrapper)
		{
			return getReader(commandText, CommandType.StoredProcedure, commandParameters, connection, cmdWrapper, (CTransaction)null);
		}

		public DbDataReader getReader(string commandText, CDBParameters commandParameters, DbConnection connection, CCommandWrapper cmdWrapper, CTransaction transaction)
		{
		...
		/// or SQL query</param>
		/// <returns>A result set of records</returns>
		public DbDataReader getReader(string commandText, CommandType commandType, CDBParameters commandParameters)
		{
			return getReader(commandText, commandType, commandParameters, (CCommandWrapper)null);
		}
		public DbDataReader getReader(string commandText, CommandType commandType, CDBParameters commandParameters, CCommandWrapper cmdWrapper)
		{
			return getReader(commandText, commandType, commandParameters, (DbConnection)null, cmdWrapper, (CTransaction)null);
		}

		public DbDataReader getReader(string commandText, CommandType commandType, CDBParameters commandParameters, CCommandWrapper cmdWrapper, CTransaction transaction)
		{
		...
		/// <param name="connection">Connection on which command should be run</param>
		/// <returns>A result set of records</returns>
		public DbDataReader getReader(string commandText, CommandType commandType, CDBParameters commandParameters, DbConnection connection, CCommandWrapper cmdWrapper, CTransaction transaction)
		{
		...
		private void CheckIfDBPackageError(Exception exc, string packageName)
		{
			if (exc == null)
				return;
		...
		public bool IsCriticalDBError { get; set; }
		...
		public int doSql(string commandText)
		{
		...
		/// <param name="connection">Connection on which command should be run</param>
		/// <returns>Number of affected rows</returns>
		public int doSql(string commandText, DbConnection connection)
		{
		...
		/// or CommandType.Text</param>
		/// <returns>Number of affected rows</returns>
		public int doSql(string commandText, CommandType commandType)
		{
		...
		public int doSql(string commandText, CommandType commandType, DbConnection connection)
		{
		...
		/// <param name="commandParameters">Parameters values for stored procedure/function</param>
		/// <returns>Number of affected rows</returns>
		public int doSql(string commandText, CDBParameters commandParameters)
		{
		...
		/// <param name="connection">Connection on which command should be run</param>
		/// <returns>Number of affected rows</returns>
		public int doSql(string commandText, CDBParameters commandParameters, DbConnection connection)
		{
		...
		/// or SQL query</param>
		/// <returns>Number of affected rows</returns>
		public int doSql(string commandText, CommandType commandType, CDBParameters commandParameters)
		{
			return doSql(commandText, commandType, commandParameters, (DbConnection)null, (CCommandWrapper)null);
		}

		public int doSql(string commandText, CommandType commandType, CDBParameters commandParameters, string transactionID)
		{
		...
		/// <param name="connection">Connection on which command should be run</param>
		/// <returns>Number of affected rows</returns>
		public int doSql(string commandText, CommandType commandType, CDBParameters commandParameters, DbConnection connection, CCommandWrapper cmdWrapper)
		{
			int result = -1;
			DbCommand command = null;
			if (connection == null)
		...
		public Thread beginClose(ref ExampleCallback callback, params object[] args)
		{
		...
		/// <returns>New thread.</returns>
		public Thread beginGetReader(ref ExampleCallback callback, params object[] args)
		{
			return callInThread(ref callback, args);
		}
		...
		/// <returns>New thread.</returns>
		public Thread beginGetScalar(ref ExampleCallback callback, params object[] args)
		{
			return callInThread(ref callback, args);
		}
		...
		/// <returns>New thread.</returns>
		public Thread beginDoSql(ref ExampleCallback callback, params object[] args)
		{
			return callInThread(ref callback, args);
		}
		...
		public void CommitTransaction(string connectionID)
		{
		...
		/// Rallback a transaction.
		/// </summary>
		public void RollbackTransaction(string connectionID)
		{
			if (mvTransactionConnections.ContainsKey(connectionID))
		...
		/// Open a transaction
		/// </summary>
		public string OpenTransaction2()
		{
			string res = String.Empty;
		...
		public string RegisterForChanges(string commandText, DependencyDelegate handler)
		{
			return RegisterForChanges(commandText, CommandType.StoredProcedure, (CDBParameters)null, handler, (DbConnection)null);
		}
		public void UnregisterForChanges(string key)
		{
			UnregisterForChanges(key, (DbConnection)null);
		}
		public void UnregisterForChanges(string key, DbConnection connection)
		{
			try
			{
		...
		private Dictionary<string, object> modDependencies = new Dictionary<string, object>();

		public string RegisterForChanges(string commandText, CommandType commandType, DependencyDelegate handler)
		{
			return RegisterForChanges(commandText, commandType, (CDBParameters)null, handler);
		}
		public string RegisterForChanges(string commandText, CommandType commandType, CDBParameters commandParameters, DependencyDelegate handler)
		{
			return RegisterForChanges(commandText, commandType, commandParameters, handler, (DbConnection)null);
		}
		public string RegisterForChanges(string commandText, CommandType commandType, CDBParameters commandParameters, DependencyDelegate handler, DbConnection connection)
		{
			string res = string.Empty;
			DbCommand command = null;
			try
		...
		public bool IsCorrectClientInstalled()
		{
			try
			{
		...
		private bool mvIsTraceNeedOn = false;
		private bool mvIsTraceOn = false;

		public bool IsTraceNeedOn
		{
		...
		public bool SetDatabaseTraceAction(bool isTraceStart, CTransaction transaction)
		{
			return SetDatabaseTraceAction(isTraceStart, transaction, null);
		}

		public bool SetDatabaseTraceAction(bool isTraceStart, DbConnection connection)
		{
			return SetDatabaseTraceAction(isTraceStart, null, connection);
		}

		public bool SetDatabaseTraceAction(bool isTraceStart, CTransaction transaction, DbConnection connection)
		{
			bool isResult = false;
			try
			{
		...
		public bool SetDatabaseTraceForAllConnection(bool isStart)
		{
			bool isResult = false;
			try
			{
		...
		public bool SetDatabaseTraceForDefaultConnection(bool isTraceStart)
		{
			bool isResult = false;
			try
			{
		...
		public string GetDatabaseTraceDirectoryPath()
		{
		...
		{
			doSql(procedureName, CommandType.StoredProcedure);
		}
	}
}